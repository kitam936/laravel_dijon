...
<x-app-layout>
    <x-slot name="header">
        ...
        <span class="items-center text-sm mt-1 text-gray-800 dark:text-gray-200 leading-tight" >　※Brand・会社を選択してください　　　</span>
            ...
            <table class="w-full bg-white table-auto md:w-1/2 text-center whitespace-no-wrap">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td class="w-1/5 pr-2 md:px-4 py-1 text-right"><span style="font-variant-numeric:tabular-nums"> {{ number_format($season_stock->pcs)}}</span></td>
                        <td class="w-2/5 pr-10 md:px-4 py-1 text-right"><span style="font-variant-numeric:tabular-nums"> {{ number_format(round($season_stock->zaikogaku)/1000)}}</span></td>
                    </tr>
                    @endforeach
                </tbody>
            </table>
        ...
        <div class=" mx-auto sm:px-4 lg:px-4 border">
            ...
            <tbody>
                ...
                <tr>
                    <td class="w-2/5 md:px-4 py-1">{{ $season_stock->season_name }}</td>
                    <td class="w-1/5 pr-2 md:px-4 py-1 text-right"><span style="font-variant-numeric:tabular-nums"> {{ number_format($season_stock->pcs)}}</span></td>
                    <td class="w-2/5 pr-10s md:px-4 py-1 text-right"><span style="font-variant-numeric:tabular-nums"> {{ number_format(round($season_stock->zaikogaku)/1000)}}</span></td>
                </tr>
                @endforeach
            </tbody>
            </table>
